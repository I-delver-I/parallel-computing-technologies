    ...
    public static void run(int matrixSize, int mainRank, int fromMainTag, int fromWorkerTag) {
        ...
        if (currentRank == mainRank) {
            ...
            for (var destRank = 1; destRank <= workersCount; destRank++) {
                ...
                MPI.COMM_WORLD.Send(new int[]{offset}, 0, 1, MPI.INT, destRank, fromMainTag);
                MPI.COMM_WORLD.Send(subMatrixA1D, 0, rowsPerWorker * matrixSize,
                        MPI.DOUBLE, destRank, fromMainTag + 1);
                MPI.COMM_WORLD.Send(matrixB1D, 0, matrixSize * matrixSize,
            ...
            for (var sourceRank = 1; sourceRank <= workersCount; sourceRank++) {
                ...
                MPI.COMM_WORLD.Recv(offset, 0, 1, MPI.INT, sourceRank, fromWorkerTag);
            ...
            System.out.println("Execution time of blocking: " + (System.currentTimeMillis() - startTime) + " ms for "
                    + workersCount + " workers");
        }
        else {
            var offset = new int[1];

            MPI.COMM_WORLD.Recv(offset, 0, 1, MPI.INT, mainRank, fromMainTag);
            var subMatrixA1D = new double[rowsPerWorker * matrixSize];
            MPI.COMM_WORLD.Recv(subMatrixA1D, 0, rowsPerWorker * matrixSize,
                    MPI.DOUBLE, mainRank, fromMainTag + 1);
            ...
            MPI.COMM_WORLD.Send(offset, 0, 1, MPI.INT, mainRank, fromWorkerTag);
            MPI.COMM_WORLD.Send(MatrixUtilities.convertMatrixToArray(resultSubMatrix), 0,
